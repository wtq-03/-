...
def main():
    ...
    print(f"找到 {len(title_to_file)} 个章节文件")
    ...
    for idx, chapter_title in enumerate(ordered_chapters, 1):
        ...
        if found_file:
            ...
            del title_to_file[found_file]  # 删除已使用的，防止重复
            success_count += 1
        else:
    ...
    print(f"第一个文件: {sorted([f for f in os.listdir('.') if f.endswith('.txt')])[0]}")
    print(f"最后一个文件: {sorted([f for f in os.listdir('.') if f.endswith('.txt')])[-1]}")
