...
MOBILE_URL = "https://m.fanqienovel.com"

# 番茄小说APP用户代理 - 使用更真实的APP User-Agent
APP_USER_AGENTS = [
...
def search_novel_mobile():
    """在移动版搜索小说"""
    print(f"在番茄小说移动版搜索《{NOVEL_NAME}》...")
    
    # 先访问移动版首页，获取必要的cookie和token
    print("访问移动版首页...")
    home_html = get_html(MOBILE_URL)
    if not home_html:
        print("无法访问移动版首页")
        return []
    ...
    search_url = f"{MOBILE_URL}/search/{search_term}"
    ...
    # 保存搜索结果到文件，方便分析
    with open('tomato_mobile_search.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print("搜索结果已保存到 tomato_mobile_search.html")
...
def get_chapter_list(novel_url):
    ...
    # 保存章节列表页面
    with open('tomato_mobile_chapters.html', 'w', encoding='utf-8') as f:
        f.write(html)
    print("章节列表已保存到 tomato_mobile_chapters.html")
